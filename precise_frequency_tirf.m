...
for jj=1:p_num
    re0_temp=finv(1,jj)*noiseimagef(:,:,jj)+re0_temp;
    rep_temp=finv(2,jj)*noiseimagef(:,:,jj)+rep_temp;
...
for jj=1:2
    ft_max=max(max(abs(re_f(:,:,jj))));
    re_f(:,:,jj)=re_f(:,:,jj)/ft_max;
    %         norm_ft(:,:,ii,jj)=re_f(:,:,ii,jj)./(n_factor+abs(re_f(:,:,ii,jj)));
    norm_ft(:,:,jj)=re_f(:,:,jj).*G;
end
...
center_x=-shiftvalue(2,1);
center_y=-shiftvalue(2,2);%
mytemp=sum(sum(conj(norm_ft(:,:,1)).*circshift(squeeze(norm_ft(:,:,2)),[center_x,center_y])));
test_ini_phase(1)=angle(mytemp);
temp_value_max=mytemp;
temp_phase(1,search_angle_num+1)=angle(mytemp);
while temp_search>threshold
    for kk=1:search_angle_num
        temp=exact_shift(squeeze(norm_ft(:,:,2)),[center_x+cos_angle(kk)*temp_search,...
            center_y+sin_angle(kk)*temp_search],1);
        mytemp=sum(sum(conj(norm_ft(:,:,1)).*temp));%
    ...
    if my_num<search_angle_num+0.5
        ...
        test_ini_phase(1)=temp_phase(my_num);
        temp_phase(1,kk+1)=temp_phase(my_num);
    end
    temp_search=temp_search*0.5;
end

precise_shift(2,1)=-center_x;
precise_shift(2,2)=-center_y;
precise_shift(3,1)=center_x;
precise_shift(3,2)=center_y;
